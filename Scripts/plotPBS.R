...
plotGenes<-function(chr,min.pos,max.pos){
  ...
  filein="references/refGene.txt.gz"
...
fin=args[1]
fout=args[2]
start=as.numeric(args[3]) # in Mbp
...
cat("Maximum PBS value:", max(pbs$PBS2[which(pbs$midPos>(start*1e6) & pbs$midPos<(end*1e6))], na.rm=T), "\n")

plot(pbs$midPos/1e6, pbs$PBS2, type ="l", col="orange",ylab = "PBS", xlab=paste("Chromosome",pbs$chr[1]), main="PBS scan" , lwd=1, xaxt="n",cex.main = 0.9, cex.axis = 0.6, cex.lab = 0.68, ylim=c(0, max(pbs$PBS2, na.rm=T)), xlim=c(start, end) ) #orange
